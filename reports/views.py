...
def report_list(request):
    ...
    - Activity audit logs with real-time updates and date/category filters
    """
...
def api_activity_logs(request):
    """
    Returns the latest activity logs as JSON with category, action_type, and date filters.
    """
    category_filter = request.GET.get("category", "").strip().upper()
    action_type_filter = request.GET.get("action_type", "").strip().upper()
    date_from = request.GET.get("date_from", "").strip()
